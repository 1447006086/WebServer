首先完成一个工作,可以根据浏览器地址栏中通过改变抽象路径部分请求到服务器webapps下对应网络应用中的
某个页面
思路:
...
上诉操作完成后,会出现一个新情况:如果地址栏中抽象路径输入的地址在webapps下没有对应的文件时,服务端
在读取该Flie表示的文件就会抛出异常.但实际上,当浏览器输入的抽象路径没有对应资源时,这件事典型的应当
返回404状态码的情况了
因此我们要完成响应404的操作

实现:
1:在webapps下新建一个子目录root
  这个目录存放所有共有的页面比如404页面,无论用户将来请求哪个网络应用下的资源都可能出现该资源不存在
  的情况,因此都应当响应404页面
2:在root下新建页面404.html
  在该页面中居中显示一行字:404,资源不存在!
3:在ClientHandler处理请求的环节,当根据抽象路径去webapps下定义资源后(实例化FIle对象),添加分支
  如果该资源存在则响应,如果该资源不存在,则响应404
  3.1:状态代码要改为404,状态描述为NotFound
  3.2:响应头Content-Length的长度为404页面的长度
  3.3:响应正文发送的应当是404.HTML这个页面的内容